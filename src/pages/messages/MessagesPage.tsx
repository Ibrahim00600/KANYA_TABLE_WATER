...
import { MessageSquare, Send, Users, Bell, Inbox } from 'lucide-react';
...
export default function MessagesPage() {
  ...
  const [compose, setCompose] = useState({
    recipient_id: '', subject: '', body: '', is_broadcast: false,
  });
  ...
  const load = useCallback(async () => {
    ...
    if (tab === 'inbox') query = query.or(`recipient_id.eq.${profile.id},is_broadcast.eq.true`);
    else if (tab === 'sent') query = query.eq('sender_id', profile.id);
    else query = query.eq('is_broadcast', true);
  ...
  async function handleSend() {
    ...
    if (!compose.is_broadcast && !compose.recipient_id) { setSendError('Select a recipient or send as broadcast.'); return; }
    setSending(true);
    setSendError('');
    const { error } = await supabase.from('messages').insert({
      sender_id: profile!.id,
      recipient_id: compose.is_broadcast ? null : compose.recipient_id,
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        ...
        <Button icon={<Send className="w-4 h-4" />} onClick={() => { setShowCompose(true); setSendError(''); }}>
          Compose
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title="No messages"
          description={tab === 'inbox' ? "Your inbox is empty." : "You haven't sent any messages."}
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <AvatarCircle name={sender?.full_name || 'Unknown'} size="sm" />
